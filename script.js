...
var model = {
	boardSize: 7,                                         //Эти три свойства помогают обойтись без жестко фиксированных значений: boardSize (размер игрового поля), numShips (количество кораблей в игре) и shipLength (длина корабля в клетках, 3).
	numShips: 3,
	shipLength: 3,
	...
	ships: [                                              //Свойство ships содержит массив объектов ship, содержащих массивы locations и hits для каждого из кораблей.
		{locations: [0, 0, 0], hits: ["", "", ""]},       //В свойстве locations хранится массив всех клеток, занимаемых кораблем.
		{locations: [0, 0, 0], hits: ["", "", ""]},       //Свойство hits содержит массив с информацией о попаданиях выстрелов в клетки. Изначально элементы массива инициализируются пустой строкой и заменяются строкой “hit" при попадании в соответствующую клетку.
		{locations: [0, 0, 0], hits: ["", "", ""]}        //координаты представляются двумя цифрами (0 = A, 1 = B и т. д.).
	],
...
function init() {                                                     //Вызовется только после полной загрузки страницы
