...
var controller = {
	...
	processGuess: function (guess) {                                //метод получающий координаты в формате “A0".
		var location = parseGuess(guess);                            //Метод parseGuess используется для проверки введенных данных.
		if (location) {                                              //Если метод не возвращает null, значит, был получен действительный объект location.
			...
			var hit = model.fire(location);                            //Затем комбинация строки и столбца передается методу fire. Напомним, что метод fire возвращает true при попадании в корабль.
...
// Отслеживание нажатий обработчиками
